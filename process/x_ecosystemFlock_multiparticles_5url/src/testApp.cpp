...
void testApp::update(){
    ...
    for (int i = 0; i < myParticles3.size(); i++){
        ...
        for (int j = 0; j < i; j++){
			...
			if ((bitFlagW_pta & bitFlagW_ptb) && (bitFlagH_pta & bitFlagH_ptb)){
				myParticles3[i].addRepulsionForce( myParticles3[j],15,1.2); 
    ...
    if(ofGetFrameNum()%200 == 0){
        ...
        ofxJSONElement trend3 = result3["count"]["approved"];
        newNum3 = trend3.asInt()/100;
        
        if (newNum3>oldNum3) {
            int numDiff3 = newNum3 - oldNum3;
            newNum3 += numDiff3;
        }
        else {
            newNum3 = oldNum3;
    ...
    for (int i = 0; i < myParticles.size(); i++){
            ...
            myParticles[i].cohesion.strength = 0.05;
            myParticles[i].alignment.strength = 0.05;
            myParticles[i].seperation.strength = 0.05;
